...
.then((res)=>{console.log(res),setCartProduct(res.data)})
.catch((err)=>{
  console.log(err)
})
  },[])
  ...
  <div key={item.id} className='flex gap-5 items-center mt-5'>
                <div className='w-20 h-20 bg-gray-100 rounded-[5px]'>
                    <img src={item.thumbnail} alt="product image" />
                </div>
                <div>
                    <h2 className='text-lg font-medium text-black font-poppins'>{item.title}</h2>
                    <p className='text-[12px] font-medium text-black font-poppins'>{item.price}</p>
                </div>
            </div>
                   )) 
                   }
         ...
         <div className='flex justify-between'>
            ...
            <p className='text-[16px] font-medium font-poppins text-black'>{cartProduct.total}</p>
         </div>
